...
const std::array<int, 10> scanList{14, 18, 28, 0, 2, 4, 6, 8, 10, 22};
...
void run_signal(const std::shared_ptr<mach::State> st) {
  ...
  while (!token.stop_requested()) {
    ...
    if ((now - old_time) > 0) {
      file << now << "," << st->lj.p10val << "," << st->lj.p20val << ","
           << st->lj.p30val << "," << st->lj.p21val << "," << st->lj.p31val
           << "," << st->lj.p22val << "," << st->lj.p32val << ","
           << st->lj.t1val << "," << st->lj.t2val << "\n";
      old_time = now;
    }
  }
  file.close();
...
int main() {
  ...
  try {
    ...
    err = LJM_eWriteNames(handle, int(sharedState->lj.p20->params.size()),
                          mach::vectorToChar(sharedState->lj.p20->params),
                          mach::vectorToDouble(sharedState->lj.p20->settings),
                          &errorAddress);
    ErrorCheck(err, "LJM_eWriteNames");
    err = LJM_eWriteNames(handle, int(sharedState->lj.p30->params.size()),
    ...
    err = LJM_eWriteNames(handle, int(sharedState->lj.p32->params.size()),
                          ...
                          &errorAddress);
    ErrorCheck(err, "LJM_eWriteNames");
    //  err = LJM_eWriteNames(handle, int(sharedState->lj.t1->params.size()),
    //  mach::vectorToChar(sharedState->lj.t1->params),
    //  mach::vectorToDouble(sharedState->lj.t1->settings), &errorAddress);
    //  ErrorCheck(err, "LJM_eWriteNames");
...
//  threads.emplace_back(waitPause, handle);
  ...
  err = LJM_eWriteName(handle, "AIN9_RESOLUTION_INDEX", 0);
  err = LJM_eWriteName(handle, "AIN9_RANGE", 0.1);
  err = LJM_eStreamStart(handle, spr, numaddr, scanListC, scanRate);
  ErrorCheck(err, "LJM_eStreamStart");
  while (!token.stop_requested()) {
    int devicebacklog;
    int ljmbacklog;
    auto *aData = new double[10];
    mach::udouble ljbits{};
    ...
    sharedState->lj.p20val = (aData[4] * 300);
    sharedState->lj.p21val = (aData[5] * 300);
    sharedState->lj.p22val = (aData[9] * 300);
    sharedState->lj.p32val = (aData[6] * 300);
    sharedState->lj.p30val = (aData[8] * 300);
    sharedState->lj.p10val = (aData[7] * 300);
